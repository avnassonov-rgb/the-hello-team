...
async function findActiveRealizationByOrderCode(orderCode) {
  ...
  try {
    ...
    return { found: false, number: null, error: e.message };
  }
}
...
async function createRealizationDocument(opts) {
  ...
  function buildBody(orgFieldName, posted) {
    const body = {
      ...
      // LineNumber — НАСТОЯЩАЯ причина общей HTTP 500: подтверждено через
      // GET /api/onec/metadata?entity=РеализацияТоваровУслуг_Товары_RowType —
      // у строки табличной части это поле Nullable="false" (единственное
      // обязательное поле строки, кроме самого Ref_Key), а мы его вообще не
      // передавали. Стандартная форма 1С нумерует строки автоматически (1,2,3…),
      // через "сырой" OData POST это нужно делать самим.
      Товары: (lines || []).map((l, i) => ({
        LineNumber: i + 1,
...
module.exports = {
  ...
  findActiveRealizationByOrderCode,
